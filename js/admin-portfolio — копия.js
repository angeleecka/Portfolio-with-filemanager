...
document.addEventListener("DOMContentLoaded", () => {
  // 🚧 Выполняем код только на страницах админки (без жёсткого #someId)
  const adminRoot =
  ...
  // --- 2. Логика фокуса для инпутов (без изменений) ---
  const inputRenameOld = document.getElementById("renameOld");
  const inputDeleteName = document.getElementById("deleteName");

  // lastActiveInput может быть не объявлен — подстрахуемся (можно оставить)
  window.lastActiveInput = window.lastActiveInput || null;

  inputRenameOld?.addEventListener("focus", () => {
    window.lastActiveInput = inputRenameOld;
  });
  inputDeleteName?.addEventListener("focus", () => {
    window.lastActiveInput = inputDeleteName;
  });

  // --- 3. Инициализация ресайзера ---
  // attachResizer() теперь будет работать, так как resizer, previewPane и workspace определены
  typeof attachResizer === "function" && attachResizer();

  // --- 4. Рендеринг и навешивание кликов ---
  // Упрощаем логику: renderPortfolio() САМ вызывает attachPreviewHandlers() в конце.
  // Нам нужно только вызвать renderPortfolio, если он существует.
  if (typeof renderPortfolio === "function") {
    renderPortfolio();
  }

  // ПРИМЕЧАНИЕ: Если renderPortfolio не существует, то attachPreviewHandlers()
  // НЕ вызывается. Если вам нужно, чтобы он вызывался и без рендеринга (например,
  // для статического контента), используйте:
  /*
  if (typeof renderPortfolio === "function") {
      renderPortfolio(); 
  } else {
      // Запасной вызов, если нет динамического рендеринга
      typeof attachPreviewHandlers === "function" && attachPreviewHandlers();
  }
  */
  // НО, так как у вас контент динамический, оставьте только вызов renderPortfolio()
  // и убедитесь, что attachPreviewHandlers() вызывается внутри renderPortfolio().
});
